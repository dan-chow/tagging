...
static int cb(enum nf_conntrack_msg_type type,
	...
	if(proto != TCP_PROTO || get_tcp_state(ct) != TCP_CONNTRACK_ESTABLISHED)
	...
	/*why printf cause segment fault??*/
	//printf("orig: srcip=%s,dstip=%s,srcport=%u,dstport=%u,bytes=%ld\n",inet_ntoa(orig_src),inet_ntoa(orig_dst),orig_l4src,orig_l4dst,orig_bytes);
